...
import abi from "./contracts/UmarNFT.json";
import nftMetaData from "./nft_metadata.json";

function App() {
  const contractAddress = '0x9189d820fB1C901DC6333eeAc06247be1A5D0605';
  ...
  const mintToken = async (tokenId) => {
    try {
      if (window.ethereum) {
        ...
        const signer = provider.getSigner();
        const nftContract = new ethers.Contract(contractAddress, contractABI, signer);
        const metadataURI = `https://gateway.pinata.cloud/ipfs/QmP6uJ1HJNWLiarU4exjdtGJgikphnQtgshMtUT1UKuJN4/${tokenId}.json`
